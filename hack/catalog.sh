...
if [ -n "$OPENSHIFT_CI" ]; then
  # Workaround for https://issues.redhat.com/browse/DPTP-1182
  export IMAGE_KNATIVE_OPERATOR="${RELEASE_IMAGE_LATEST%/*}/stable:knative-operator}"
  export IMAGE_KNATIVE_OPENSHIFT_INGRESS="${RELEASE_IMAGE_LATEST%/*}/stable:knative-openshift-ingress}"

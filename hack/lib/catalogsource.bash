...
function install_catalogsource {
  ...
  if [ -n "$OPENSHIFT_CI" ] || [ -n "$DOCKER_REPO_OVERRIDE" ]; then
    ...
    if [[ "${OPENSHIFT_BUILD_NAME:-}" = serverless-operator-src* ]]; then
      ...
      sed -i "s,image: .*openshift-serverless-.*:knative-operator,image: ${SERVERLESS_KNATIVE_OPERATOR}," "$csv"
      sed -i "s,image: .*openshift-serverless-.*:knative-openshift-ingress,image: ${SERVERLESS_INGRESS}," "$csv"
      sed -i "s,image: .*openshift-serverless-.*:openshift-knative-operator,image: ${SERVERLESS_OPENSHIFT_KNATIVE_OPERATOR}," "$csv"

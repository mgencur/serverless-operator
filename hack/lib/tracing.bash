...
function enable_eventing_tracing {
  ...
  oc -n "${EVENTING_NAMESPACE}" patch knativeeventing/knative-eventing --type=merge --patch='{"spec": {"config": { "tracing": {"enable":"true","backend":"zipkin", "zipkin-endpoint":"'"${endpoint}"'", "sample-rate":"0.1"}}}}'
}
...
function enable_serving_tracing {
  ...
  local endpoint
  endpoint=$(get_tracing_endpoint)
  oc -n "${SERVING_NAMESPACE}" patch knativeserving/knative-serving --type=merge --patch='{"spec": {"config": { "tracing": {"enable":"true","backend":"zipkin", "zipkin-endpoint":"'"${endpoint}"'", "sample-rate":"0.1"}}}}'
}
